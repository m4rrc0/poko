...
n2m.setCustomTransformer('child_page', (blockRaw) => {
	return `<ChildPageData blockRaw={${JSON.stringify(blockRaw)}} />`;
});

n2m.setCustomTransformer('child_database', (blockRaw) => {
	return `<CollectionData blockRaw={${JSON.stringify(blockRaw)}} />`;
});

n2m.setCustomTransformer('link_to_page', (blockRaw) => {
	return `<LinkToPageData blockRaw={${JSON.stringify(blockRaw)}} />`;
});

// n2m.setCustomTransformer('synced_block', (blockRaw) => {
// 	console.log(blockRaw);
// 	// return `<LinkToPageData blockRaw={${JSON.stringify(blockRaw)}} />`;
// });

// n2m.setCustomTransformer('column_list', (blockRaw) => {
// 	// console.log(blockRaw);
// 	return `<Columns blockRaw={${JSON.stringify(blockRaw)}} />`;
// });
...
export async function getBlockChildrenRecursively(
	...
	if (Array.isArray(blocksOrBlock)) {
		...
		if (blocksOrBlock.object === 'page' || blocksOrBlock.object === 'database') {
	...
	const blocks = await Promise.all(
		_blocks.map(async (_block) => {
			...
			const linkToPageId = block?.link_to_page?.page_id || block?.link_to_page?.database_id;
			const childPageTitle = block?.child_page?.title || block?.child_database?.title;

			if (syncedBlockId) {
				children = await getBlockChildrenRecursively(syncedBlockId, allPages, currentPageId, true);
				...
				if (allPages) {
					...
					if (pages[0].object === 'database') {
						...
						subChildren = await getBlockChildrenRecursively(pages[0], allPages, currentPageId);
					}
				...
				children = await getBlockChildrenRecursively(block, allPages, currentPageId);
			}
...
export function transformRichTextToPlainText(_val) {
	...
	if (Array.isArray(_val)) return _val.map(({ plain_text }) => plain_text).join('');
...
export const dicoNotionBlockRoles = {
	paragraph: 'p',
	heading_1: 'h1',
	heading_2: 'h2',
	heading_3: 'h3',
