...
export async function get() {
	...
	const globalStylesBlocks = blocks.filter((b) => b?.code?.language === 'css');
	// const globalStylesBlock = blocks.find((b) => b?.code?.language === 'css');

	let globalStylesString = globalStylesBlocks
	...
	await postcss(postCssConfig.plugins)
		.process(globalStylesString, { from, to: from })
		.then((result) => {
